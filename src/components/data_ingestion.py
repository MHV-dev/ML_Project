# Data from multiple sources like servers,db's,hudoops,cloud etc
# Access and read the data from diferent sources
...
@dataclass                            #Decorator->define class variable directly(normally done using {__init__})
...
class DataIngestion :
    ...
    def initiate_data_ingestion(self):
        ...
        try:
            ...
            df=pd.read_csv('C:\\Users\\harsh\\python programs\\ML_Project\\src\\notebooks\\data\\student.csv')
            logging.info('Read the dataset as dataframe')

            os.makedirs(os.path.dirname(self.injestion_config.train_data_path),exist_ok=True)

            df.to_csv(self.injestion_config.raw_data_path,index=False,header=True) # Save raw data
